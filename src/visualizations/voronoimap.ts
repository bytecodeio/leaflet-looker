...
const vis: VoronoiMap = {
    ...
    update: function (data, element, config, queryResponse) {
        ...
        addPoints(simplePointData(data),
            map,
            config.pointColor,
            config.pointRadius,
            data,
...
const findSourceDataPoint = (latlng, fullData) => {
    ...
    return fullData.find((point) => filterString === point['users.location'].filterable_value);
    };
...
function addPoints(data, map, pointColor, pointRadius, fullData, lineColor){
    ...
    var popupContent = (latlng, fullData) => (
        '<h3> Source data: </h3>' +
        '<pre>' + JSON.stringify(findSourceDataPoint(latlng, fullData), null, 2) + '</pre>'
    );

    pointsLayer = L.layerGroup(data.map(function(v){
        return L.circleMarker(L.latLng(v),{
		  ...
		  .bindPopup((x) => popupContent(x._latlng, fullData),
            popupOptions)
    }));
